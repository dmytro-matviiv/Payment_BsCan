...
from config import WALLET_ADDRESS, CHECK_INTERVAL, MIN_AMOUNT_USDT, TOKEN_SYMBOL, MAX_BLOCKS_PER_CHECK
...
class PaymentMonitorBot:
    ...
    def init_start_block(self):
        ...
        if self.start_block:
            ...
            print(f"⚠️ Увага: Бот перевіряє тільки останні {MAX_BLOCKS_PER_CHECK} блоків за раз")
    ...
    def check_new_transactions(self):
        ...
        # Перевіряємо останні MAX_BLOCKS_PER_CHECK блоків (або менше, якщо нових блоків менше) для економії API credits
        start_block = max(self.start_block + 1, latest_block - (MAX_BLOCKS_PER_CHECK - 1))
        ...
        try:
            ...
            # Не оновлюємо start_block, щоб спробувати знову наступного разу
            return
        ...
        for tx in transactions:
            try:
                ...
                try:
                    ...
                    print(f"⚠️ Помилка форматування транзакції {tx_hash[:16]}...: {format_error}")
                    continue
                
                # Перевіряємо токен
                if formatted_tx.get('symbol', '').upper() != TOKEN_SYMBOL.upper():
                    continue
                
                # Перевіряємо мінімальну суму
                amount = formatted_tx.get('amount', 0)
                if amount < MIN_AMOUNT_USDT:
                    print(f"⏭️ Транзакція {tx_hash[:16]}... сума {amount:.2f} < {MIN_AMOUNT_USDT}")
                    continue
                
                new_incoming.append(tx)
            except Exception as tx_error:
                print(f"⚠️ Помилка обробки транзакції: {tx_error}")
                continue
        ...
        for tx in new_incoming:
            try:
                ...
                print(f"\n💸 НОВА ОПЛАТА!")
                print(f"   Хеш: {tx_hash}")
                print(f"   Сума: {formatted_tx.get('amount', 0):.2f} {formatted_tx.get('symbol', 'USDT')}")
                print(f"   Від: {formatted_tx.get('from_address', 'N/A')}")
                print(f"   Час: {formatted_tx.get('timestamp', 'N/A')}")
                ...
                try:
                    ...
                    print(f"   ❌ Помилка надсилання в Telegram: {telegram_error}")
                    # Все одно додаємо транзакцію до оброблених, щоб не повторювати
                    self.processed_txs.add(tx_hash)
            except Exception as tx_error:
                print(f"⚠️ Помилка обробки транзакції {tx.get('hash', 'unknown')[:16]}...: {tx_error}")
                continue
    ...
    def run(self):
        ...
        if CHECK_INTERVAL >= 60:
            ...
            print(f"⏱️ Інтервал: {interval_min:.1f} хвилин ({CHECK_INTERVAL} секунд)")
        else:
