...
REQUEST_DELAY = 0.5  # Затримка між запитами до блоків (секунди)
MAX_RETRIES = 5  # Максимальна кількість спроб при 429 помилці
RETRY_BASE_DELAY = 2.0  # Базова затримка для retry (секунди, буде збільшуватися експоненційно)
MAX_RETRY_DELAY = 60.0  # Максимальна затримка для retry (секунди)